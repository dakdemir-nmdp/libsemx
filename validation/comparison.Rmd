...
# Fit using LBFGS
fit_glmm <- semx_fit(model_glmm, sleepstudy, optimizer_name = "lbfgs", estimation_method = "ML")
...
fe_semx <- c(
  Intercept = get_param(params_semx, "alpha_Reaction_on__intercept"),
...
# semx returns Cholesky factors for random effects covariance
# L = [L00, 0; L10, L11]
# stored as cov_re_1_0 (L00), cov_re_1_1 (L10), cov_re_1_2 (L11)
L00 <- get_param(params_semx, "cov_re_1_0")
L10 <- get_param(params_semx, "cov_re_1_1")
L11 <- get_param(params_semx, "cov_re_1_2")

re_var_int_semx <- L00^2
re_cov_semx <- L00 * L10
re_var_slope_semx <- L10^2 + L11^2
...
model_sem <- semx_model(
  equations = c(
    ...
    "textual =~ x4 + x5 + x6",
    "speed   =~ x7 + x8 + x9",
    # Explicitly add intercepts for indicators
    "x1 ~ 1", "x2 ~ 1", "x3 ~ 1",
    "x4 ~ 1", "x5 ~ 1", "x6 ~ 1",
    "x7 ~ 1", "x8 ~ 1", "x9 ~ 1",
    # Explicitly add covariances between latent factors
    "visual ~~ textual",
    "visual ~~ speed",
    "textual ~~ speed"
  ),
