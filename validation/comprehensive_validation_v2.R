...
# 12. Survival Analysis (Weibull) vs survival::survreg ----------------
...
if (requireNamespace("survival", quietly = TRUE)) {
  ...
  coef_survreg <- coef(fit_survreg_weibull)
  scale_survreg <- fit_survreg_weibull$scale
  
  expect_equal(as.numeric(params_weibull["beta_futime_on_age", "Estimate"]), as.numeric(coef_survreg["age"]), tolerance = 1e-3)
  expect_equal(as.numeric(params_weibull["beta_futime_on_rx", "Estimate"]), as.numeric(coef_survreg["rx"]), tolerance = 1e-3)
  expect_equal(as.numeric(params_weibull["alpha_futime_on__intercept", "Estimate"]), as.numeric(coef_survreg["(Intercept)"]), tolerance = 1e-3)
  
  psi_est <- as.numeric(params_weibull["psi_futime_futime", "Estimate"])
  expect_equal(psi_est, 1/scale_survreg, tolerance = 1e-2)
  
  summary_rows <- c(summary_rows, list(capture_row("weibull", c(
    beta_age = params_weibull["beta_futime_on_age", "Estimate"],
    beta_rx = params_weibull["beta_futime_on_rx", "Estimate"],
    intercept = params_weibull["alpha_futime_on__intercept", "Estimate"],
    shape = psi_est
  ))))
} else {
  cat("skipping Weibull survival: package survival not available\n")
}
...
if (requireNamespace("survival", quietly = TRUE)) {
  ...
  coef_survreg_exp <- coef(fit_survreg_exp)
  
  expect_equal(as.numeric(params_exp["beta_futime_on_age", "Estimate"]), as.numeric(coef_survreg_exp["age"]), tolerance = 1e-3)
  expect_equal(as.numeric(params_exp["beta_futime_on_rx", "Estimate"]), as.numeric(coef_survreg_exp["rx"]), tolerance = 1e-3)
  expect_equal(as.numeric(params_exp["alpha_futime_on__intercept", "Estimate"]), as.numeric(coef_survreg_exp["(Intercept)"]), tolerance = 1e-3)
  
  summary_rows <- c(summary_rows, list(capture_row("exponential", c(
    beta_age = params_exp["beta_futime_on_age", "Estimate"],
    beta_rx = params_exp["beta_futime_on_rx", "Estimate"],
    intercept = params_exp["alpha_futime_on__intercept", "Estimate"]
  ))))
} else {
  cat("skipping Exponential survival: package survival not available\n")
}

# 14. Fit Indices Check -----------------------------------------------
cat("\n--- Test 14: Fit Indices Check ---\n")
if (exists("fit_semx_cfa")) {
  summ_cfa <- summary(fit_semx_cfa)
  indices <- summ_cfa$fit_indices
  
  expect_true(!is.null(indices$cfi))
  expect_true(!is.null(indices$tli))
  expect_true(!is.null(indices$rmsea))
  expect_true(!is.null(indices$srmr))
  
  if (!is.nan(indices$cfi)) expect_gt(indices$cfi, 0.9)
  if (!is.nan(indices$srmr)) expect_lt(indices$srmr, 0.1)
  
  cat("CFI:", indices$cfi, "TLI:", indices$tli, "RMSEA:", indices$rmsea, "SRMR:", indices$srmr, "\n")
} else {
  cat("skipping Fit Indices Check: CFA model not available\n")
}
